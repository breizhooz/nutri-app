...
def create_user():
    with httpx.Client() as client:
        ...
        yield user
        login = client.post(f"{SERVICE_USER_URL}/api/v1/auth/login", json=_RECIPE_USER)
...
def auth_token(create_user):
    with httpx.Client() as client:
        resp = client.post(f"{SERVICE_USER_URL}/api/v1/auth/login", json=_RECIPE_USER)
...
def test_recipe_health():
    with httpx.Client() as client:
        response = client.get(f"{SERVICE_RECIPE_URL}/health")
    assert response.status_code == 200
    assert response.json()["status"] == "ok"
    assert response.json()["service"] == "service-recipe"


@pytest.mark.smoke
def test_recipe_health_db():
    with httpx.Client() as client:
        response = client.get(f"{SERVICE_RECIPE_URL}/health/db")
    assert response.status_code == 200
