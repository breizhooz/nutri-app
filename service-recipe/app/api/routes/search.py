...
async def search_recipe(
    ...
    exclude_allergens: Optional[list[str]] = Query(None, description="Allergene à exclure (enum key)"),
    exclude_diets: Optional[list[str]] = Query(None, description="Diet à exclure (enum key)"),
    exclude_nutrition: Optional[list[str]] = Query(None, description="Tag nutritionnels à excluse (enum key)"),
    limit: int = Query(10, ge=1, le=100),
    offset: int = Query(0, ge=0)
):
    """
        Full-text recipe search with optional filters.
