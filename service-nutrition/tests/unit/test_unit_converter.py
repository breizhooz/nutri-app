...
class TestUnitConverter:
    ...
    async def test_grammes_passthrough(self, db_session):
        assert await UnitConverter(db_session).to_grammes(200.0, "g") == pytest.approx(
            200.0
        )

    @pytest.mark.unit
    async def test_kilogrammes(self, db_session):
        assert await UnitConverter(db_session).to_grammes(0.5, "kg") == pytest.approx(
            500.0
        )

    @pytest.mark.unit
    async def test_millilitres(self, db_session):
        assert await UnitConverter(db_session).to_grammes(100.0, "ml") == pytest.approx(
            100.0
        )

    @pytest.mark.unit
    async def test_litres(self, db_session):
        assert await UnitConverter(db_session).to_grammes(1.5, "l") == pytest.approx(
            1500.0
        )

    @pytest.mark.unit
    async def test_centilitres(self, db_session):
        assert await UnitConverter(db_session).to_grammes(5.0, "cl") == pytest.approx(
            50.0
        )

    @pytest.mark.unit
    async def test_decilitres(self, db_session):
        assert await UnitConverter(db_session).to_grammes(2.0, "dl") == pytest.approx(
            200.0
        )

    @pytest.mark.unit
    async def test_case_insensitive_g(self, db_session):
        assert await UnitConverter(db_session).to_grammes(100.0, "G") == pytest.approx(
            100.0
        )

    @pytest.mark.unit
    async def test_case_insensitive_kg(self, db_session):
        assert await UnitConverter(db_session).to_grammes(1.0, "KG") == pytest.approx(
            1000.0
        )
    ...
    async def test_specific_aliment_type_takes_priority(self, db_session):
        ...
        result = await UnitConverter(db_session).to_grammes(
            1.0, "cs", aliment_type="farine"
        )
