...
def _to_pdf(sl: ShoppingList) -> StreamingResponse:
    ...
    pdf.cell(
        0,
        12,
        f"Liste de courses - semaine du {sl.start_date}",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    for cat_key, items in groups.items():
        ...
        pdf.cell(
            col_unit, 7, "Unité", border=1, fill=True, new_x="LMARGIN", new_y="NEXT"
        )
        ...
        for i, item in enumerate(items):
            ...
            pdf.cell(
                col_qty, 7, str(item.total_quantity), border="LR", fill=True, align="R"
            )
            pdf.cell(
                col_unit,
                7,
        ...
        pdf.cell(
            col_ingredient + col_qty + col_unit,
            0.3,
            "",
            new_x="LMARGIN",
            new_y="NEXT",
