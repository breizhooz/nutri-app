...
class TestCreateMenu:
    ...
    async def test_nb_persons_zero_rejected(self, client: AsyncClient):
        assert (
            await client.post(BASE, json=menu_payload(nb_persons=0))
        ).status_code == 422

    async def test_slots_created_inline(self, client: AsyncClient):
        body = (
            await client.post(BASE, json=menu_payload(slots=[slot_payload()]))
        ).json()
    ...
    async def test_exclusions_stored(self, client: AsyncClient):
        body = (
            await client.post(
                BASE, json=menu_payload(exclusions=["enums.allergen.gluten"])
            )
        ).json()
    ...
    async def test_free_tags_stored(self, client: AsyncClient):
        body = (
            await client.post(
                BASE, json=menu_payload(free_tags={"occasion": "birthday"})
            )
        ).json()
    ...
    async def test_notes_stored(self, client: AsyncClient):
        body = (
            await client.post(BASE, json=menu_payload(notes="Vegetarian week"))
        ).json()
...
class TestUpdateMenu:
    async def test_updates_notes(self, client: AsyncClient):
        created = (await client.post(BASE, json=menu_payload())).json()
        body = (
            await client.put(f"{BASE}/{created['id']}", json={"notes": "Updated"})
        ).json()
    ...
    async def test_updates_nb_persons(self, client: AsyncClient):
        created = (await client.post(BASE, json=menu_payload())).json()
        body = (
            await client.put(f"{BASE}/{created['id']}", json={"nb_persons": 4})
        ).json()
    ...
    async def test_replaces_slots_when_provided(self, client: AsyncClient):
        ...
        body = (
            await client.put(f"{BASE}/{created['id']}", json={"slots": new_slots})
        ).json()
        assert len(body["slots"]) == 2
    ...
    async def test_partial_update_leaves_other_fields_unchanged(
        ...
        created = (
            await client.post(BASE, json=menu_payload(notes="Original", rating=3))
        ).json()
        body = (
            await client.put(f"{BASE}/{created['id']}", json={"notes": "Changed"})
        ).json()
        assert body["rating"] == 3
    ...
    async def test_404_for_nonexistent(self, client: AsyncClient):
        assert (
            await client.put(f"{BASE}/99999", json={"notes": "x"})
        ).status_code == 404
...
class TestDeleteMenu:
    ...
    async def test_slots_cascade_deleted(
        ...
        result = await session.execute(
            sa_select(MenuSlot).where(MenuSlot.menu_id == menu_id)
        )
