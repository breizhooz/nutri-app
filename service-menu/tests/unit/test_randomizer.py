...
class TestHasExcludedAllergen:
    ...
    def test_no_recipe_ingredients(self):
        assert (
            _has_excluded_allergen(
                {"recipe_ingredients": []}, {"enums.allergen.gluten"}
            )
            is False
        )
    ...
    def test_multiple_allergens_one_matches(self):
        ...
        assert (
            _has_excluded_allergen(
                recipe, {"enums.allergen.gluten", "enums.allergen.milk"}
            )
            is True
        )
...
class TestGenerateSlots:
    ...
    async def test_caloric_soft_filter_applied_when_pool_large_enough(self):
        high = [
            make_recipe(i, cals_per_100g=1000.0, qty=100, servings=1)
            for i in range(1, 50)
        ]
        low = [
            make_recipe(i + 100, cals_per_100g=50.0, qty=100, servings=1)
            for i in range(1, 50)
        ]
        slots = await generate_slots(
