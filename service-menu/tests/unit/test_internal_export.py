"""Tests de l'endpoint interne d'export RGPD (art. 20) — service-menu."""
...
@pytest.mark.unit
async def test_export_returns_menus_and_slots(service_client, session):
    """L'export renvoie les menus du compte ciblé, créneaux imbriqués."""
    acc = str(uuid.uuid4())
    ...
    resp = await service_client.post(
        "/api/v1/internal/export", json={"account_ids": [acc]}
    )
    assert resp.status_code == 200
    menus = resp.json()["data"]["weekly_menus"]
    assert len(menus) == 1
    assert menus[0]["slug"] == "menu-a"
    assert len(menus[0]["slots"]) == 1


@pytest.mark.unit
async def test_export_empty_when_no_menu(service_client):
    """Sans menu, l'export renvoie une liste vide (200)."""
    resp = await service_client.post(
        "/api/v1/internal/export", json={"account_ids": [str(uuid.uuid4())]}
    )
    assert resp.status_code == 200
    assert resp.json()["data"]["weekly_menus"] == []


@pytest.mark.unit
async def test_export_requires_service_token(session):
